...
- `name` : 학생 실명.
- `student_number` : 학교에서 부여하는 고유 학번, 중복 불가.
...
**컬럼 설명**
- `session_id` : 주차별 실제 수업 식별자 (PK).
- `schedule_id` : 어떤 반복 슬롯에서 파생됐는지 FK.
- `class_date` : 실제 수업 날짜.
- `is_cancelled` : 휴강 여부 (TRUE 시 출석 미집계).
...
- `subject.professor_id` → `professor.professor_id` (`ON DELETE SET NULL`)
- `subject_schedule.subject_id` → `subject.subject_id` (`ON DELETE CASCADE`)
- `enrollment.student_id` → `student.student_id` (`ON DELETE CASCADE`)
- `enrollment.subject_id` → `subject.subject_id` (`ON DELETE CASCADE`)
- `class_session.schedule_id` → `subject_schedule.schedule_id` (`ON DELETE CASCADE`)
- `checkin.session_id` → `class_session.session_id` (`ON DELETE CASCADE`)
- `checkin.student_id` → `student.student_id` (`ON DELETE CASCADE`)
